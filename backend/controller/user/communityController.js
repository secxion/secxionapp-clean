...
export const submitNewPostController = async (req, res) => {
  if (!req.userId) {
    return res
      .status(401)
      .json({ message: "Unauthorized! Please login to post.", success: false });
  }
  ...
  if (!content) {
    return res
      .status(400)
      .json({ message: "Post content is required.", success: false });
  }
...
export const deletePostController = async (req, res) => {
  if (!req.userId) {
    return res
      .status(401)
      .json({
        message: "Unauthorized! Please login to delete your post.",
        success: false,
      });
  }
