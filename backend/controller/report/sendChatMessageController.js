    ...
    try {
        ...
        try {
            await createReportReplyNotification(
                report.userId,
                report._id.toString(),
                message ? `New reply from moderator: ${message}` : 'New image reply from moderator.',
                `/reports/${report._id}`
            );
        } catch (error) {
        ...
        if (notificationError) {
            ...
            console.warn("Report message sent successfully, but notification creation failed.", notificationError);
            return res.json({
                message: "Message sent successfully, but there was an issue creating the notification.",
                success: true,
                error: false,
                notificationError: {
                    message: notificationError.message || "Failed to create notification.",
                    details: notificationError,
                },
            });
        }

        return res.json({ message: "Message sent successfully.", success: true, error: false });
