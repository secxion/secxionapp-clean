...
export const getUserTransactions = async (req, res) => {
    try {
        ...
        const transactionHistoryWithPossibleUpdatedStatus = await Promise.all(
            transactions.map(async (tx) => {
                ...
                if (tx.onModel === 'PaymentRequest' && tx.referenceId) {
                    try {
                        const paymentRequest = await PaymentRequest.findById(tx.referenceId);
                        if (paymentRequest) {
        ...
        let filteredTransactionHistory = transactionHistoryWithPossibleUpdatedStatus;
        ...
        filteredTransactionHistory.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
