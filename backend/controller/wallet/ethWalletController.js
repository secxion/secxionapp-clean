...
export const saveEthWalletAddress = async (req, res) => {
  try {
    ...
    if (!ethAddress) {
      return res
        .status(400)
        .json({ success: false, message: "ETH address is required." });
    }
    ...
    res
      ...
      .json({
        success: false,
        message: "Error saving ETH address",
        error: err.message,
      });
  }
};
...
export const getUserEthWallet = async (req, res) => {
  try {
    ...
    if (!wallet) {
      return res
        .status(404)
        .json({ success: false, message: "ETH Wallet not found" });
    }
    ...
    res
      ...
      .json({
        success: false,
        message: "Failed to fetch ETH wallet",
        error: error.message,
      });
  }
};
...
export const withdrawEth = async (req, res) => {
  try {
    ...
    if (!toAddress || !amountEth) {
      return res
        .status(400)
        .json({ success: false, message: "Address and amount required" });
    }
    ...
    res
      ...
      .json({
        success: true,
        message: `Withdrawal request to ${toAddress} for ${amountEth} ETH submitted.`,
      });
  } catch (err) {
