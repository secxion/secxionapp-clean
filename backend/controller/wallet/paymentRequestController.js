...
export const createPaymentRequest = async (req, res) => {
    try {
        ...
        if (!walletUpdateResult.success) {
            console.error('Error debiting wallet after payment request:', walletUpdateResult.error);
        } else {
...
export const updatePaymentRequestStatus = async (req, res) => {
    try {
        ...
        if (!['pending', 'approved-processing', 'rejected', 'completed'].includes(status)) {
            return res.status(400).json({
                success: false,
        ...
        if (rejectionReason && status === 'rejected') {
            ...
            if (!refundResult.success) {
                console.error('Error crediting wallet after rejecting payment request:', refundResult.error);
            } else {
                await createTransactionNotification(
                    userId,
                    amount,
        ...
        if (status === 'approved-processing') {
            ...
            if (!walletUpdateResult.success) {
                return res.status(500).json({
                    success: false,
                    message: 'Failed to update wallet transaction status after approving request.',
