      ...
      {reports.map((report) => (
        <div key={report._id} className="bg-white shadow rounded-md p-4 mb-4">
          <div className="flex justify-between items-center mb-2">
            ...
            <span
              className={`text-xs px-2 py-1 rounded-full ${report.status === 'Pending' ? 'bg-yellow-500 text-white' : report.status === 'Resolved' ? 'bg-green-500 text-white' : 'bg-red-500 text-white'}`}
            >
              {report.status}
            </span>
          </div>
          <p className="text-gray-600 text-sm mb-2">
            Submitted by: {report.name} ({report.email}) on{' '}
              ...
              <div className="overflow-y-auto h-64 p-2 bg-gray-100 rounded-md mb-2">
                ...
                chatMessages[report._id].length > 0 ? (
                  chatMessages[report._id].map((msg, index) => (
                    <div
                      key={index}
                      className={`mb-2 p-2 rounded-md ${msg.sender === 'admin' ? 'bg-blue-100 text-blue-700 self-end text-right' : 'bg-gray-200 text-gray-700 self-start text-left'}`}
                    >
