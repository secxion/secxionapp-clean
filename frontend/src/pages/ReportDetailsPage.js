...
const ReportDetailsPage = () => {
    ...
    useEffect(() => {
        const fetchReportDetails = async () => {
            ...
            setError('');
            try {
                const response = await fetch(`${SummaryApi.fetchReportDetail.url}/${reportId}`, {
    ...
    if (!report) {
        return <div className="text-gray-500 text-center py-10">Report not found.</div>;
    }
                    ...
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        ...
                        {report.image && (
                            <div className="sm:col-span-2">
                                ...
                                <dd className="mt-1 text-sm text-gray-900">
                                    <img src={report.image} alt="Report Attachment" className="max-w-full h-auto rounded-md border" />
                                </dd>
                            </div>
                        )}
                            ...
                            <div className="sm:col-span-2 border-t border-gray-200 pt-4">
                                ...
                                {report.adminReplyImage && (
                                    <dd className="mt-2 text-sm text-gray-900">
                                        <img src={report.adminReplyImage} alt="Admin Reply Attachment" className="max-w-full h-auto rounded-md border" />
                                    </dd>
                                )}
                                <p className="mt-1 text-xs text-gray-500">
                                    Replied At: {format(new Date(report.updatedAt), 'yyyy-MM-dd HH:mm:ss')}
                                </p>
                            </div>
                        )}
                        {!report.adminReply && !report.adminReplyImage && report.status === 'Resolved' && (
                            <div className="sm:col-span-2 border-t border-gray-200 pt-4">
                                <dt className="text-sm font-medium text-yellow-500">
                                    Reply Pending Display
                                </dt>
                                <dd className="mt-1 text-sm text-gray-700">
                                    The administrator's reply will be displayed here shortly. Please refresh if you don't see it.
                                </dd>
                            </div>
                        )}
                    </div>
