    ...
    >
      ...
      <div className="shape-lines relative bg-gray-900 bg-opacity-95 p-6 sm:p-8 mt-10 w-full max-w-md rounded-2xl shadow-2xl z-10">
        ...
        <button
          ...
          className={`flex items-center gap-2 text-yellow-400 hover:text-yellow-200 font-semibold transition-colors focus:outline-none ${bubbleIn ? "bubble-pop" : ""}`}
          aria-label="Go back"
          type="button"
          style={getBubbleStyle(bubbleIn, 1)}
        >
          <ArrowLeft className="w-5 h-5" style={getBubbleStyle(bubbleIn, 2)} />
          <span style={getBubbleStyle(bubbleIn, 3)}>Back</span>
        </button>
