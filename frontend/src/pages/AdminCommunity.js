...
const AdminCommunity = () => {
    ...
    if (loading) return <div className="min-h-screen bg-gray-100 dark:bg-gray-900 flex items-center justify-center">Loading pending posts...</div>;
    if (error) return <div className="min-h-screen bg-gray-100 dark:bg-gray-900 flex items-center justify-center text-red-500">Error loading pending posts: {error}</div>;

    if (!user) {
        return <div className="min-h-screen bg-gray-100 dark:bg-gray-900 flex items-center justify-center text-red-500">Please log in to access this page.</div>;
    }
                ...
                {pendingPosts.length > 0 ? (
                    <ul>
                        {pendingPosts.map(post => (
                            <li key={post._id} className="mb-4 p-4 bg-white dark:bg-gray-800 shadow rounded-md border-b border-gray-200 dark:border-gray-700">
                                <div className="flex items-start space-x-3">
                                    {post.userId?.profilePict && (
                                        <img src={post.userId.profilePic} alt={post.userId.name} className="w-8 h-8 rounded-full object-cover" />
                                    )}
                                    {!post.userId?.profilePic && (
                                        <div className="w-8 h-8 rounded-full bg-gray-400 flex items-center justify-center text-white text-sm">{post.userId?.name?.charAt(0).toUpperCase()}</div>
                                    )}
                                    <div>
                                        <p className="font-semibold text-gray-900 dark:text-gray-100">{post.userId?.name || 'Anonymous'}</p>
                                        <p className="text-gray-600 dark:text-gray-400 text-sm">{post.content}</p>
                                        <p className="text-gray-600 dark:text-gray-400 text-sm mt-2 rounded-md max-w-full h-auto">{post.feedImage}</p>

                                        {post.feedImage && (
                                            <img src={post.feedImage} alt={post.feedImage} className="mt-2 rounded-md max-w-full h-auto" />
                                        )}
                    ...
                    <p className="text-gray-600 dark:text-gray-400 text-center">No pending community posts.</p>
                )}
