...
const AdminPanel = () => {
    ...
    const menuItems = [
        ...
        { path: "all-products", label: "Products", icon: <FaBoxOpen className="mr-2" /> },
        ...
        <div className="container min-h-screen bg-gray-100">
            ...
            <div className="flex items-center cursor-pointer" onClick={toggleUserInfo}>
                    ...
                    <div>
                        ...
                        {isUserInfoExpanded && <p className="text-xs text-gray-600">{user?.role}</p>}
                    </div>
                    <div className="ml-2">
                        {isUserInfoExpanded ? <ChevronUpIcon className="h-5 w-5 text-gray-500" /> : <ChevronDownIcon className="h-5 w-5 text-gray-500" />}
                    </div>
                </div>
                        ...
                        <li key={path}>
                            <button
                                ...
                                className={`px-4 py-2 rounded-md text-sm font-medium whitespace-nowrap focus:outline-none flex items-center ${
                                    location.pathname.includes(path) ? 'bg-blue-500 text-white shadow-sm' : 'text-gray-700 hover:bg-gray-100'
                                }`}
                            >
                                {icon}
                                {label}
