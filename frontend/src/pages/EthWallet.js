...
const Notification = ({ type, message, onDismiss }) => {
  ...
  return (
    <div className={`${bgColor} ${textColor} p-4 mb-4 rounded-md shadow-lg flex items-center justify-between animate-fadeIn`}>
      ...
      {onDismiss && (
        <button onClick={onDismiss} className="ml-4 p-1 rounded-full hover:bg-black/20 transition-colors" aria-label="dismiss notification">
...
const EthWallet = () => {
  ...
  const formatTime = (date) => date.toLocaleTimeString([], { hour12: false, hour: "2-digit", minute: "2-digit", second: "2-digit", timeZoneName: "short" });
  ...
  const renderCountdown = () => {
    ...
    return <div className="mt-3 text-yellow-600 font-semibold">Processing: {mins}m {secs}s</div>;
  };
  ...
  const svcPercent = parseFloat(serviceFeePercent) || SERVICE_FEE_PERCENT;
  const svcAmountDisplay = exactEthEquivalent ? (exactEthEquivalent * (svcPercent / 100)).toFixed(6) : "0.000000";
  ...
  return (
    <div className="p-10 max-w-full mt-16 mx-auto sm:p-6 md:p-8 max-w-2xl sm:mt-16 text-gray-200 bg-gray-800 shadow-2xl border border-gray-700">
      ...
      {errorMessage && <Notification type="error" message={errorMessage} onDismiss={dismissError} />}
      {successMessage && <Notification type="success" message={successMessage} onDismiss={dismissSuccess} />}

      {/* Wallet Info Section */}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4 mb-6 p-4 bg-gray-900/50 rounded-lg border border-gray-700">
        {[
          { Icon: CurrencyDollarIcon, label: "ETH Rate (NGN)", value: ethRate ? `₦${parseFloat(ethRate).toLocaleString("en-NG", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : "...", color: "text-green-400" },
          { Icon: FireIcon, label: "Est. Gas Fee (ETH)", value: `${gasFeeFloat.toFixed(6)} ETH`, color: "text-red-400" },
          { Icon: CreditCardIcon, label: "Naira Balance", value: nairaBalance ? `₦${parseFloat(nairaBalance).toLocaleString("en-NG", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : "...", color: "text-yellow-300" },
          { Icon: CubeTransparentIcon, label: "ETH Balance", value: ethBalance ? `${parseFloat(ethBalance).toFixed(6)} ETH` : "...", color: "text-blue-400" },
      ...
      <form onSubmit={(e) => { e.preventDefault(); handleWithdrawRequest(); }} className="space-y-6">
        ...
        <div className="p-4 bg-gray-900/50 rounded-lg space-y-3 text-sm border border-gray-700">
          ...
          <p className="flex justify-between items-center text-gray-300">
            ...
            <span className="font-medium text-red-300">{gasFeeFloat.toFixed(6)} ETH</span>
          </p>
          <p className="flex justify-between items-center text-gray-300">
            <span>Service Fee ({svcPercent}%):</span>
            <span className="font-medium text-yellow-300">{svcAmountDisplay} ETH</span>
          </p>
          <p className="flex justify-between items-center text-gray-300">
        ...
        <button
          ...
          disabled={withdrawLoading || countdown > 0 || withdrawalStatus === "pending" || isInitialLoading || !nairaWithdrawAmount || parseFloat(nairaWithdrawAmount) <= 0 || !ethAddress || (exactEthToSend !== null && exactEthToSend <= 0)}
        >
          {withdrawLoading ? <ArrowPathIcon className="h-5 w-5 animate-spin" /> : <CreditCardIcon className="h-5 w-5" />}
          <span>{withdrawLoading ? "Processing..." : (countdown > 0 || withdrawalStatus === "pending") ? "Request Pending" : "Request ETH Withdrawal"}</span>
        </button>
      </form>

      {lastUpdated && (
        <p className="mt-8 text-xs text-gray-500 text-center">
          <ClockIcon className="h-3.5 w-3.5 inline mr-1 align-middle" /> Wallet data last updated: {lastUpdated}
        </p>
      )}
    </div>
  );
