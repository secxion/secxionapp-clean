          ...
          <div className="flex justify-between items-center">
            <div className="flex items-center space-x-8">
              <button
                ...
                className="inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl border border-white/10 bg-white/5 p-0 text-gray-400 transition-all duration-300 hover:bg-white/10 hover:text-brand-gold md:hidden"
            ...
            <div className="flex items-center gap-6">
              {activeTab !== 'wallet' && (
                <button
                  onClick={() => setShowBalance(!showBalance)}
                  className="bg-white/5 border border-white/10 rounded-2xl px-6 py-2.5 backdrop-blur-xl group hover:border-brand-gold/30 transition-all duration-300 active:scale-95"
                  title={showBalance ? 'Hide balance' : 'Show balance'}
                >
                  <div className="flex items-center space-x-4">
                    <FaWallet
                      className={`w-3.5 h-3.5 ${showBalance ? 'text-brand-gold' : 'text-gray-500'}`}
                    />
                    <span className="text-emerald-400 font-black font-spaceGrotesk text-sm tracking-tight">
                      {showBalance
                        ? `₦${(walletBalance || 0).toLocaleString()}`
                        : '••••••••'}
                    </span>
                    {showBalance ? (
                      <FaEyeSlash className="w-3 h-3 text-gray-500 group-hover:text-white" />
                    ) : (
                      <FaEye className="w-3 h-3 text-gray-500 group-hover:text-white" />
                    )}
                  </div>
                </button>
              )}
