    ...
    const handleUpdatePricing = (currencyIndex, faceValueIndex, field, value) => {
        setData((prev) => {
            ...
            if (faceValueIndex !== undefined) {
                if (!updatedPricing[currencyIndex].faceValues[faceValueIndex]) return prev;
    ...
    return (
        <div className="container fixed inset-0 z-50 bg-gray-900 bg-opacity-50 flex justify-center items-center">
            <div className="bg-white p-6 rounded-md mt-40 shadow-lg w-full max-w-2xl max-h-[80vh] overflow-y-auto">
                ...
                <form className="space-y-4 overflow-y-auto pb-6" onSubmit={handleSubmit}>
                    <div>
                        <label htmlFor="productName" className="block text-sm font-medium text-gray-700">Product Name:</label>
                        <input type="text" id="productName" name="productName" value={data.productName} onChange={handleOnChange} className="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required placeholder="Enter product name" />
                    </div>
                    <div>
                        <label htmlFor="brandName" className="block text-sm font-medium text-gray-700">Brand Name:</label>
                        <input type="text" id="brandName" name="brandName" value={data.brandName} onChange={handleOnChange} className="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required placeholder="Enter brand name" />
                    </div>
                    <div>
                        <label htmlFor="category" className="block text-sm font-medium text-gray-700">Category:</label>
                        <select id="category" name="category" value={data.category} onChange={handleOnChange} className="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>
                    ...
                    <div>
                        <label htmlFor="uploadImageInput" className="block text-sm font-medium text-gray-700">Product Image:</label>
                            ...
                            <label htmlFor="uploadImageInput" className="cursor-pointer">
                                <div className="space-y-1 text-center">
                                    ...
                                    <div className="flex text-sm text-gray-600">
                                        ...
                                        <input id="uploadImageInput" name="productImage" type="file" className="sr-only" onChange={handleUploadProduct} />
                                    </div>
                        ...
                        <div className="mt-2 flex space-x-2">
                            {data.productImage.map((img, index) => (
                                <div key={index} className="relative w-20 h-20 rounded-md overflow-hidden shadow-sm">
                                    <img src={img} alt={`product-${index}`} className="object-cover w-full h-full cursor-pointer" onClick={() => { setOpenFullScreenImage(true); setFullScreenImage(img); }} />
                                    <button
                                        type="button"
                    ...
                    <div>
                        ...
                        {data.pricing.map((currency, currencyIndex) => (
                            <div key={currencyIndex} className="mb-4 border rounded p-4 bg-gray-50 shadow-sm">
                                <div className="mb-2">
                                    <label htmlFor={`currency-${currencyIndex}`} className="block text-xs font-medium text-gray-700">Currency:</label>
                                    <select
                                    ...
                                    >
                                        ...
                                        {currencyData.map((cur) => (
                                            <option value={cur.value} key={cur.value}>{cur.label}</option>
                                        ))}
                                    </select>
                                </div>
                                <div>
                                    <label className="block text-xs font-medium text-gray-700">Face Values:</label>
                                    {currency.faceValues.map((faceValue, faceValueIndex) => (
                                        <div key={faceValueIndex} className="flex gap-2 mb-2">
                                            <input
                                                ...
                                                onChange={(e) => handleUpdatePricing(currencyIndex, faceValueIndex, "faceValue", e.target.value)}
                                                className="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                                            />
                                            <input
                                                type="text"
                                                ...
                                                onChange={(e) => handleUpdatePricing(currencyIndex, faceValueIndex, "sellingPrice", e.target.value)}
                                                className="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                                            />
                                            <input
                                                type="text"
