        ...
        <div className="fixed inset-0 bg-black bg-opacity-70 flex justify-center items-start z-50 overflow-y-auto pt-12 px-4">
            <div className="w-full max-w-3xl bg-gradient-to-br from-gray-900 via-gray-800 to-gray-950 p-6 rounded-2xl shadow-2xl relative border-2 border-yellow-700">
                ...
                <div className="flex justify-between items-center border-b pb-4 mb-4 border-gray-800">
                    <h2 className="text-xl md:text-2xl font-bold text-yellow-400">📦 Upload Product Details</h2>
                    <button onClick={onClose} className="text-gray-400 hover:text-yellow-400 text-2xl">
                        <CgClose />
                    </button>
                </div>
                ...
                {productDetails.productImage && (
                    <div className="flex gap-4 items-center border border-yellow-700 rounded-lg p-4 bg-gray-950 mb-6 shadow-sm">
                        ...
                        <div className="flex flex-col gap-1 text-sm text-gray-200">
                            <p><span className="font-semibold text-yellow-400">Name:</span> {productDetails.productName}</p>
                            <p><span className="font-semibold text-yellow-400">Currency:</span>
                            ...
                            <p><span className="font-semibold text-yellow-400">Face Value:</span> {productDetails.faceValue}</p>
                            <p><span className="font-semibold text-yellow-400">Rate:</span> {productDetails.rate}</p>
                        </div>
                    </div>
                )}

                {/* Form */}
                <form className="space-y-6 overflow-y-auto max-h-[60vh] pr-2" onSubmit={handleSubmit}>
                    {/* Image Upload */}
                    <div>
                        <label className="block font-semibold text-yellow-400 mb-2">📸 Upload Additional Images</label>
                        <label htmlFor="uploadInput" className={`flex flex-col items-center justify-center border-2 border-yellow-700 rounded-lg p-5 bg-gray-950 cursor-pointer transition ${uploading ? 'opacity-50 pointer-events-none' : ''}`}>
                            <FaCloudUploadAlt className="text-4xl text-yellow-500 mb-1" />
                            <p className="text-gray-200">{uploading ? 'Uploading...' : 'Click or drag to upload'}</p>
                        </label>
                        ...
                        <div className="flex gap-3 mt-4 flex-wrap">
                            {data.Image.map((img, idx) => (
                                <div key={idx} className="relative group">
                                    ...
                                    }} className="w-20 h-20 rounded-md border border-yellow-700 object-cover cursor-pointer hover:scale-105 transition" alt={`Uploaded product image ${idx + 1}`} />
                    ...
                    <div>
                        <label className="block font-semibold text-yellow-400 mb-2">💰 Total Face Value ({currencySymbol})</label>
                        <input
                            type="number"
                            name="totalAmount"
                    ...
                    <div>
                        <label className="block font-semibold text-yellow-400 mb-2">= Calculated Total Amount:</label>
                    ...
                    {/* New Code Input */}
                    <div>
                        <label className="block font-semibold text-yellow-400 mb-2">Code</label>
                        <input
                            type="text"
                            name="cardcode"
                            value={data.cardcode}
                    ...
                    {/* Remarks */}
                    <div>
                        <label className="block font-semibold text-yellow-400 mb-2">📝 Additional Remarks</label>
                        <textarea
                            name="userRemark"
                            rows={4}
