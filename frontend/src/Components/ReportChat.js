...
const ReportChat = ({ category, newReport, setNewReport }) => {
        ...
        try {
            ...
            if (data.success && Array.isArray(data.data)) {
                const filteredReports = data.data.filter(
                    (report) => report.category === category || (category && report.message === category)
                );
                ...
                const allChatMessages = filteredReports.reduce((acc, report) => {
                    if (report.chatHistory && Array.isArray(report.chatHistory)) {
                        return [...acc, ...report.chatHistory.map(msg => ({ ...msg, reportId: report._id }))];
                    }
                ...
                if (newReport && (newReport.category === category || newReport.message === category)) {
                    const newMessage = {
                    ...
                    setMessages(allChatMessages.sort((a, b) => new Date(a.createdAt) - new Date(b.createdAt)));
                }
            } else {
                setMessages([]);
            }
    ...
    const handleSendMessage = async () => {
        ...
        try {
            ...
            const response = await fetch(SummaryApi.userReplyReport.url.replace(":id", newMessage.reportId), {
    ...
    const getUserDisplay = (senderName) => {
        if (senderName && user?.name === senderName && user?.profilePicture) {
            return <img src={user.profilePicture} alt="Profile" className="w-8 h-8 rounded-full mr-2 object-cover" />;
        } else if (senderName && user?.name === senderName) {
            return <div className="w-8 h-8 rounded-full bg-gray-400 text-white flex items-center justify-center mr-2">{user.name.charAt(0).toUpperCase()}</div>;
                ...
                {fetchingMessages ? (
                    ...
                    messages.map((msg, index) => (
                        ...
                        >
                            ...
                            <div className={`rounded-lg p-2 break-words ${msg.sender === 'user' ? 'bg-blue-200 text-blue-800' : 'bg-gray-300 text-gray-800'}`}>
                                <p className="text-sm">{msg.message}</p>
                                {msg.image && (
                                    <img src={msg.image} alt="Attachment" className="max-w-xs h-auto rounded-md mt-1" />
                                )}
                                <p className="text-xs text-gray-500 mt-1">{format(new Date(msg.createdAt), "yyyy-MM-dd HH:mm:ss")}</p>
                            </div>
            ...
            <div className="p-3 border-t">
                <div className="flex items-center">
                    ...
                    <label className="cursor-pointer mr-2">
                        ...
                        <input type="file" className="hidden" onChange={handleMessageImageUpload} />
                    </label>
                    <button
                ...
                {messageImage && (
                    <div className="relative mt-2 inline-block">
                        <img src={messageImage} alt="To Send" className="w-20 h-20 object-cover rounded-md" />
                        <button
