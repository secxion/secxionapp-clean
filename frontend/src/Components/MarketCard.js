            ...
            <div className="mt-2">
                ...
                {market.pricing && market.pricing.length > 0 ? (
                    market.pricing.map((price, index) => (
                        <div key={price._id || index} className="text-gray-600 border-b pb-2 mb-2">
                                ...
                                <div className="mt-1">
                                    ...
                                    {price.faceValues.map((face, i) => (
                                        <div key={i} className="ml-2 text-gray-700">
                                            <p>🔹 <strong>Face Value:</strong> {face.faceValue}</p>
                                            <p>💰 <strong>Rate:</strong> {face.sellingPrice}</p>
                                            {face.description && <p>📝 <strong>Description:</strong> {face.description}</p>}
                                        </div>
                                    ))}
                                </div>
                            ) : (
